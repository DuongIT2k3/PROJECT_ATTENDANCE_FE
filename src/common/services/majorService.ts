...
export const getAllMajor = async(params?: Params): Promise<IResponse<Major[]>> => {
    const res = await apiClient.get("/majors", { params });
    return res.data;
}

export const createMajor = async(payload: Omit<Major, "_id" | "deletedAt" | "createdAt" | "updatedAt">): Promise<Major> => {
    const res = await apiClient.post("/majors", payload);
    return res.data.data;
}

export const updateMajor = async (
    id: string, payload: Partial<Omit<Major, "_id" | "deletedAt" | "createdAt" | "updatedAt">>
): Promise<Major> => {
...
export const softDeleteMajor = async (id: string): Promise<Major> => {
    const res = await apiClient.delete(`/majors/soft/${id}`);
    return res.data.data;
}
